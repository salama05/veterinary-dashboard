...
const Suppliers = () => {
    ...
    const handlePaymentSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingPaymentId) {
                await api.put(`/suppliers/${selectedSupplier._id}/payment/${editingPaymentId}`, paymentData);
            } else {
            ...
            fetchSuppliers();
            // Update selected supplier locally if viewing details
            const { data } = await api.get('/suppliers');
            setSuppliers(data);
            if (isDetailModalOpen) {
                const updated = data.find((s: any) => s._id === selectedSupplier._id);
    ...
    const handlePaymentDelete = async (paymentId: string) => {
        ...
        if (window.confirm('هل أنت متأكد من حذف هذه الدفعة؟')) {
            try {
                await api.delete(`/suppliers/${selectedSupplier._id}/payment/${paymentId}`);
                // Update lists
                const { data } = await api.get('/suppliers');
                setSuppliers(data);
                if (isDetailModalOpen) {
    ...
    const openDetails = (supplier: any) => {
    ...
    return (
        <div>
                ...
                <div className="flex gap-2 w-full md:w-auto items-center">
                    ...
                    <ExportMenu
                        ...
                        columns={[
                            ...
                            { key: 'totalPurchases', label: 'إجمالي المشتريات', formatter: (val) => val?.toLocaleString() || '0' },
                    ...
                    <table className="w-full text-right">
                        <thead className="bg-gray-50/50 dark:bg-gray-700/30 text-gray-600 dark:text-gray-300 border-b border-gray-100 dark:border-gray-700">
                            <tr>
                                ...
                                <th className="p-5 font-bold text-sm">إجمالي المشتريات</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                            {filteredSuppliers.map((supplier) => (
                                <tr key={supplier._id} className="group hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors duration-200">
                                    <td className="p-5 font-medium text-gray-800 dark:text-white">{supplier.name}</td>
                                    <td className="p-5 text-gray-600 dark:text-gray-300">{supplier.address}</td>
                                    <td className="p-5 text-blue-600 font-bold">{supplier.totalPurchases?.toLocaleString()}</td>
                                    <td className="p-5 text-emerald-600 font-bold">{supplier.totalPaid?.toLocaleString()}</td>
                                    <td className="p-5">
                                        <span className={`px-2 py-1 rounded-md text-xs font-bold ${(supplier.totalRest || 0) > 0 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'
                                            }`}>
                                            {supplier.totalRest?.toLocaleString()}
                                        </span>
                                    </td>
                                    <td className="p-5">
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => openDetails(supplier)} className="p-2 text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-lg dark:bg-gray-700 dark:text-gray-300" title="التفاصيل">
                                                <Eye className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => openEdit(supplier)} className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg dark:bg-indigo-900/20 dark:text-indigo-400" title="تعديل">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => openPayment(supplier)} className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg dark:bg-emerald-900/20 dark:text-emerald-400" title="تسديد">
                                                <DollarSign className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(supplier._id)} className="p-2 text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg dark:bg-rose-900/20 dark:text-rose-400" title="حذف">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center mt-4">
                                    <div className="p-2 bg-blue-50 rounded">
                                        <p className="text-xs text-blue-600">المشتريات</p>
                                        <p className="font-bold">{selectedSupplier.totalPurchases?.toLocaleString()}</p>
                                    </div>
                                    <div className="p-2 bg-green-50 rounded">
                                        <p className="text-xs text-green-600">المدفوع</p>
                                        <p className="font-bold">{selectedSupplier.totalPaid?.toLocaleString()}</p>
                                    </div>
                                    <div className="p-2 bg-red-50 rounded">
                                        <p className="text-xs text-red-600">الباقي</p>
                                        <p className="font-bold">{selectedSupplier.totalRest?.toLocaleString()}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'payments' && (
                            <div className="overflow-y-auto max-h-[400px]">
                                <table className="w-full text-sm text-right">
                                    ...
                                    <tbody>
                                        {selectedSupplier.payments?.map((payment: any, index: number) => (
            ...
            <Modal
                ...
                title={editingPaymentId ? `تعديل دفعة: ${selectedSupplier?.name}` : `تسديد دفعة للمورد: ${selectedSupplier?.name}`}
            >
